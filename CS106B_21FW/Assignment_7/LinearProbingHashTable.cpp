...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    /* full or exist */
    if(logicalSize == allocatedSize || this->contains(elem))
    {
        return false;
    }
    else
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int index = HF(elem);
    for(int i=index,j=0; j<this->allocatedSize && elems[i].type!=SlotType::EMPTY; i=(i+1)%this->allocatedSize,j++)
    {
        if(elems[i].value == elem && elems[i].type!=SlotType::TOMBSTONE)
        {
            return true;
        }
    }
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    {
        int index = HF(elem);
        for(int i=index, j=0; j<allocatedSize && elems[i].type != SlotType::EMPTY ; i=(i+1)%this->allocatedSize,j++)
        {
