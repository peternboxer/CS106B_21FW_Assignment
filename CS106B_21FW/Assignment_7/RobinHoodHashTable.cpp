...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    this->HF = hashFn;
    this->allocatedSize = HF.numSlots();
    this->elems = new Slot[allocatedSize];
...
bool RobinHoodHashTable::insert(const string& elem) {
    /* if full or exist already */
    if(this->logicalSize==this->allocatedSize || this->contains(elem))
    {
        return false;
    }
    int index = HF(elem);
    ...
    while(1)
    {
        ...
        {
            elems[index].value = str;
            elems[index].distance = dis;
            this->logicalSize++;
            return true;
        }
        /* slot occupied and have to move on */
        if(dis<=elems[index].distance)
        {
            dis++;
            index = (index+1)%allocatedSize;
        }
        /* take the slot and displace */
        else if(dis>elems[index].distance)
        {
            //elems[index].distance = dis;
            /* displace */
            swap(dis,elems[index].distance);
            swap(str,elems[index].value);
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int index = HF(elem);
    int dis=0;
    for(int i=index, j=0;j<this->allocatedSize; i=(i+1)%this->allocatedSize)
    {
        if(elems[i].value == elem)
        {
            return true;
        }
        else if(elems[i].distance<0)
        {
            return false;
        }
        else if(elems[i].distance < dis)
        {
            return false;
        }
        dis++;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int index = HF(elem);
    for(int i=index,j=0;j<this->allocatedSize;i=(i+1)%allocatedSize,j++)
    {
        /* found */
        if(elems[i].value == elem && elems[i].distance>=0)
        {
            elems[i].distance = EMPTY_SLOT;
            elems[i].value = "";
            logicalSize--;

            while(elems[(i+1)%allocatedSize].distance>0)
            {
                swap(elems[i].value,elems[(i+1)%allocatedSize].value);
                swap(elems[i].distance,elems[(i+1)%allocatedSize].distance);
                elems[i].distance--;
                i = (i+1)%allocatedSize;
            }
            return true;
