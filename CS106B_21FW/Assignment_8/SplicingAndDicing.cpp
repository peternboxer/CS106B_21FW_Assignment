...
Nucleotide* toStrand(const string& str) {
    ...
    i++;
    Nucleotide* temp = start;
    while(i<str.size())
    {
        temp->next = new Nucleotide;
        temp->next->value = str[i];
        temp->next->prev = temp;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide* find = dna;
    Nucleotide* find_t = target;
    Nucleotide* ret = nullptr;
    int count = 0;

    while(1)
    {
        /* not match */
        if(find->value!=find_t->value)
        {
            /* reset ret, count, and find_t */
            ret = nullptr;
            find_t = target;
            ...
            /* if there is match so far, need to reset find and skip the one elemenbt from start */
            while(count >1)
            {
                find=find->prev;
                count--;
        ...
        else
        {
            ret = find;
            count++;
            /* target done, ready to report */
            if(find_t->next == nullptr)
            {
                while(count>1)
                {
                    ret=ret->prev;
            ...
            /* dna search done but target not done */
            if(find->next == nullptr)
            {
                return nullptr;
            }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    /* if target is empty, leave dna unmodified */
    if(target == nullptr)
    {
        return true;
    }

    Nucleotide* ret = findFirst(dna,target);
    ...
    /* find out how many elements in target */
    Nucleotide* find = target;
    int count = 1;
    ...
    /* if it starts from the 1st element */
    if(ret->prev==nullptr)
    {
        while(count >0)
        {
    ...
    /* if it starts in the middle */
    else
    {
        Nucleotide* l1 = ret;
        Nucleotide* l2 = nullptr;
        temp = ret->prev;
        while(count >0)
        {
            l2 = l1->next;
