...
Set<Shift> buildup(Set<Shift> &shifts, Set<Shift> &soFar,const int maxHours,int &currentHours);

bool checkOverlap(const Shift data, const Set<Shift> &set);

Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {

    int currentHours = 0;
    Set<Shift> origin = shifts;
    Set<Shift> start;
    Set<Shift> complete;
    ...
    complete = buildup(origin,start,maxHours,currentHours);
    return complete;
}
...
Set<Shift> buildup(Set<Shift> &shifts, Set<Shift> &soFar,const int maxHours,int &currentHours)
{
    ...
    /* if empty, return completed set */
    if(shifts.isEmpty())
    {
        return soFar;
    }
    /* not empty, continue building up */
    else
    {
        first = shifts.first();
        /* if overlaps */
        if(checkOverlap(first,soFar))
        {
            shifts.remove(first);
            return buildup(shifts, soFar, maxHours, currentHours );
        }
            /* if it exceeds max hours limit */
        if(currentHours+lengthOf(first) > maxHours)
        {
            shifts.remove(first);
            return buildup(shifts, soFar, maxHours, currentHours );
        }

        /*option-1 not take it */
        shifts.remove(first);
        Set<Shift>new_soFar = soFar;
        Set<Shift> new_shift = shifts;
        int new_hours = currentHours;
        temp1 = buildup(new_shift, new_soFar, maxHours, new_hours );
        /*option-2 take it */
        Set<Shift>new_soFar2 = soFar;
        Set<Shift> new_shift2 = shifts;
        new_soFar2.add(first);
        int new_hours2 = (currentHours + lengthOf(first));
        temp2 = buildup(new_shift2, new_soFar2, maxHours, new_hours2);
    }
    ...
    Set<Shift> abc = (score1 >= score2)?temp1:temp2;
    return abc;
}

/* helper function to check overlapping
   reture true if overlap
