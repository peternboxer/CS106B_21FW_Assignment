...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    /* map container to obtain charactor variation and frequency */
    Map<char,int> map;
    ...
    /* less than 2 different chatactors */
    if(map.size()<2)
    {
        error("invalid input, need at least 2 different charactors");
    }
    /* Initialize PriorityQueue and load up */
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while(!bits.isEmpty())
    {
        EncodingTreeNode* find = tree;
        /* keep going till leaf */
        while(find->zero!=nullptr || find->one!=nullptr)
        {
            Bit data = bits.dequeue();
            if(data==0)
            {
                find = find->zero;
            }
...
void codeTable(EncodingTreeNode*& tree, Queue<Bit>& q, Map<char, Queue<Bit>>& map);
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {

    Queue<Bit> qq;
    Map<char,Queue<Bit>> book;

    /* generate code table based on tree alone */
    codeTable(tree, qq, book);

    for(char single:str)
    {
        Queue<Bit> temp = book.get(single);
        while(!temp.isEmpty())
        {
            qq.enqueue(temp.dequeue());
        }
    }
    return qq;
}
...
void codeTable(EncodingTreeNode*& tree, Queue<Bit>& q, Map<char, Queue<Bit>>& map)
{
    ...
    /* reach the end */
    if(find->zero==nullptr && find->one==nullptr)
    {
        char value = find->ch;
    ...
    while(find->zero!=nullptr || find->one!=nullptr)
    {
        if(find->zero!=nullptr)
        {
            temp.enqueue(0);
            codeTable(find->zero, temp, map);
        }
        if(find->one!=nullptr)
        {
            temp = q;
            temp.enqueue(1);
            codeTable(find->one, temp, map);
        }
        return;
    }
    return;
...
HuffmanResult compress(const string& text) {
    ...
    encodeTree(tree, bits_all, char_all);
    str_all = encodeText(text, tree);

    /* assemble */
    HuffmanResult ret;
    ret.treeBits = bits_all;
    ret.treeLeaves = char_all;
    ret.messageBits = str_all;
...
string decompress(HuffmanResult& file) {
    ...
    /* unpack */
    bits_all = file.treeBits;
    char_all = file.treeLeaves;
    str_all = file.messageBits;

    /* build tree */
    EncodingTreeNode* tree = decodeTree(bits_all, char_all);

    /* decode into string */
    string ret = "";
    ret = decodeText(str_all, tree);
