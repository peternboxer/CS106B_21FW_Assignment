...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    else
    {
        ...
        {
            sum += pow(input[word],2);
        }
        if(sum==0)
        {
            error("nonzero value doesn't exist.");
        }
        sum = sqrt(sum);
        /* push into new map container */
        for(string word:input)
        {
            box[word] = input[word]/sum;
        }
    }
     return box;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    /* input is const type, return newly created empty container */
    else if(numToKeep == 0)
    {
        return box;
    }
    ...
    else
    {
        ...
        {
            top.enqueue(word,source[word]);
        }
        /* dump */
        for(int i=0;i<total-numToKeep;i++)
        {
            top.dequeue();
        }
        /* load up into new container */
        while(!top.isEmpty())
        {
            box[top.peek()]=top.peekPriority();
            top.dequeue();
        }
    }
    return box;
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    else
    {
        for(string word:lhs)
        {
            if(rhs.containsKey(word))
            {
                ret += lhs[word]*rhs[word];
            }
        }
    }
    return ret;
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    PriorityQueue<string> box;
    double ret = 0;

    for(Corpus data:corpora)
    {
        ret = cosineSimilarityOf(textProfile, data.profile);
        box.enqueue(data.name,ret);
    }

    string final;
    /* dump everything from box */
    while(!box.isEmpty())
    {
        final = box.peek();
        box.dequeue();
