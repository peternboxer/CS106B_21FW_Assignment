...
bool findPair(Map<string, Set<string>>& remain, Set<string> &chosen, Set<Pair> &ret);
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    /* recursively search all potential partners */
    for(string sub:possibleLinks[current])
    {
        int flag = 1;
        /* search through existing pairs */
        /* if matching is empty, flag = 1 */
        for(Pair sp:matching)
        {
            /* if member paired previously, break and proceed to the next one */
            if(sub==sp.first() || sub==sp.second())
            {
                flag=0;
                break;
            }
        ...
        if(flag == 1)
        {
            Pair new_pair = {current, sub};
            /* make a copy of Map for recursion */
            auto my_matching = matching + new_pair;
            auto mycopy = possibleLinks;
            /* remove paired members */
            mycopy.remove(current);
            mycopy.remove(sub);
            /* continue searching recursively */
            if(hasPerfectMatching(mycopy, my_matching))
            {
...
Set<Pair> findMW(Map<string, Map<string, int>>&remain, Set<Set<Pair>>&box, Set<Pair>&pair);
int TotalScore(const Map<string, Map<string, int>>& map, const Set<Pair> pair);

Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {

    Set<Pair> ret;

    Set<Set<Pair>> AllPairs;
    Map<string, Map<string, int>> remain = possibleLinks;

    findMW(remain, AllPairs,ret);

    /* go through completed collection and find the pair w/ highest score */
    int MaxScore = 0;
    Set<Pair> final;
    for(Set<Pair> mySet:AllPairs)
    {
        /* help function defined below */
        int score = TotalScore(possibleLinks,mySet);
        if(score >MaxScore)
        {
            MaxScore = score;
            final = mySet;
...
Set<Pair> findMW(Map<string, Map<string, int>>&remain, Set<Set<Pair>>&box, Set<Pair>&pair)
{
    /* search complete, add completed set to the collection */
    if(remain.isEmpty())
    {
        return pair;
    }

    string current = remain.firstKey();
    for(Pair sub_pair:pair)
    {
        if(current == sub_pair.first() || current == sub_pair.second())
        {
            remain.remove(current);
            return findMW(remain, box, pair);
        }
    }

    /* recursively try all potential partners */
    for(string sub:remain[current])
    {
        int flag = 1;
        /* if negative, skip */
        if((remain[current])[sub]<0)
        {
            continue;
        }
        /* check if sub is previously paired alreasdy */
        for(Pair sub_pair:pair)
        {
            if(sub == sub_pair.first() || sub == sub_pair.second())
            {
                flag = 0;
                break;
            }
        }
        /* if sub not paired previously */
        if(flag)
        {
            if(sub == "C")
            {
            ...
            /* create new pair and add to collection */
            Pair new_pair = {current, sub};
            /* new copy of pair */
            Set<Pair>pair2 = pair;
            pair2.add(new_pair);
            /* update remain map */
            Map<string, Map<string, int>> new_remain = remain;
            new_remain.remove(current);
            new_remain.remove(sub);
            /* continue building up collection recursively */
            Set<Pair> new_set = findMW(new_remain,box,pair2);
            /* add completed set into collection */
            box.add(new_set);
            /* Delete members to avoid repeating matching in the future rounds */
            remain[sub].remove(current);
        }
    }
    remain.remove(current);
    return findMW(remain, box, pair);
...
/* help function to calculate total score from a set */
int TotalScore(const Map<string, Map<string, int>>& map, const Set<Pair> pair)
{
    int score = 0;
    for(Pair sub:pair)
    {
        auto A = map[sub.first()];  // Map A
        score += A[sub.second()];
    }
    return score;
}
...
namespace {
    ...
    bool isPerfectMatching(const Map<string, Set<string>>& possibleLinks,
        ...
        bool AA = (used.size() == possibleLinks.size());
        if(AA==false)
        {
            cout<<endl;
        }
