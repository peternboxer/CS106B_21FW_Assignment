 ...
 */
bool ifCover(const Map<string, Set<string>>& roadNetwork, const Set<string> set);

bool tryCover(const Map<string, Set<string>>& roadNetwork, Set<string>&set, Set<string>&chosen, int num);
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities==0)
    {
        /* if not all cities covered, failed */
        if(roadNetwork.size()>cover.size())
        {
            return false;
        }
        return true;
    ...
    string current = roadNetwork.firstKey();
    Set<string> new_set = roadNetwork[current];
    Set<string> covered;
    new_set.add(current);

    if(roadNetwork.size()<=numCities)
    {
        for(string sub:roadNetwork)
        {
            supplyLocations.add(sub);
        }
        return true;
    }
    return tryCover(roadNetwork, new_set,supplyLocations, numCities);
}

bool tryCover(const Map<string, Set<string>>& roadNetwork, Set<string>&set, Set<string>&chosen, int num)
{
    /* if out of stock - check coverage result */
    if(num==0)
    {
        /* all cities covered*/
        if(ifCover(roadNetwork,chosen))
        {
            return true;
        }
        return false;
    ...
    for(string sub:set)
    {
        /* if this city has stockpile, skip to next */
        ...
        /* ---------------------- */
        /* For the next recursive search, we need to create a set that include:
           any one of the uncovered member + all its neighbors */
        Set<string> all_city;
        for(string sub_city:roadNetwork)
        {
            all_city += sub_city;
        }

        /* currently covered cities */
        Set<string>covered = chosen;
        for(string str:chosen)
        {
            covered += roadNetwork[str];
        }

        if(roadNetwork.size()==covered.size())
        {
            return true;
        }
        /* uncovered cities */
        Set<string> new_set = all_city - covered;
        /* pull any of the uncovered cities */
        string any = new_set.first();
        /* add all of its neighbors to the set */
        new_set.clear();
        new_set += any;
        new_set += roadNetwork[any];

        if(tryCover(roadNetwork, new_set,chosen, new_num-1))
        {
            return true;
        }
        /* if failed */
...
/* helper function to check if a city is covered */
bool ifCover(const Map<string, Set<string>>& roadNetwork, const Set<string> chosen)
{
    Set<string> cover;
    for(string sub:chosen)
    {
        cover += roadNetwork[sub];
    }
    for(string sub:chosen)
    {
        cover += sub;
