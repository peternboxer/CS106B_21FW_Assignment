...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    /* if not evey state could get at least 1 seat */
    ...
    /* initialize result map, initial seat = 1 */
    Map<string, int> ret;
    for(string sub:populations)
    {
        ret.put(sub,1);
        DataPoint dp;
        dp.name = sub;
        dp.weight = populations[sub]/sqrt(2);
        /* pull smallest and convert to largest */
        dp.weight *= (-1);
        hp.enqueue(dp);
    }

    numSeats -= populations.size();
    /* if one seat per state */
    if(numSeats == 0)
    {
        return ret;
    }
    ...
    while(numSeats>0)
    {
        DataPoint temp = hp.peek();
        /* grant one seat */
        ret[temp.name]++;
        numSeats--;
        /* determine new weight */
        temp.weight = populations[temp.name]/sqrt(ret[temp.name]*(ret[temp.name]+1));
        /* convert to negative */
        temp.weight *= (-1);
        hp.dequeue();
        hp.enqueue(temp);
