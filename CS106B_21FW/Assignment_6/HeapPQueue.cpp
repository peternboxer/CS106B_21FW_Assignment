...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    /* if run out of space, need to double the size */
    if(logicalSize+1 == allocatedSize)
    {
        DataPoint* ptr = new DataPoint[2*allocatedSize];
    ...
    int new_dp = logicalSize;

    while(new_dp>1 && elems[new_dp/2].weight>elems[new_dp].weight)
    {
        /* swap position */
        swap(elems[new_dp/2],elems[new_dp]);
        new_dp = new_dp/2;
    }
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    else
    {
        ...
        /* delete content */
        temp = elems[logicalSize];
        elems[logicalSize] = {};
        logicalSize--;

        /* bubble-down */
        int new_index = 1;
        while(logicalSize >= 2*new_index && new_index < logicalSize)
        {
            int current = 2*new_index;
            int right = current + 1;
            /* right */
            if(right <= logicalSize && elems[right].weight < elems[current].weight)
            {
                current = right;
            }
            if(elems[new_index].weight < elems[current].weight)
            {
                break;
            }

            swap(elems[new_index], elems[current]);
            new_index = current;

//            if(logicalSize == 2*new_index && elems[new_index].weight > elems[2*new_index].weight)
//            {
//                swap(elems[new_index], elems[2*new_index]);
//                return temp;
//            }
//            else if(logicalSize == 2*new_index && elems[new_index].weight <= elems[2*new_index].weight)
//            {
//                return temp;
//            }
//            /* to the left */
//            else if((elems[2*new_index].weight <= elems[2*new_index+1].weight) && elems[new_index].weight > elems[2*new_index].weight)
//            {
//                swap(elems[new_index], elems[2*new_index]);
//                new_index *= 2;
//            }
//            /* to the right */
//            else if((elems[2*new_index].weight > elems[2*new_index+1].weight) && elems[new_index].weight > elems[2*new_index+1].weight)
//            {
//                swap(elems[new_index], elems[2*new_index+1]);
