...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    {
        int half = s/2;
        Vector<Vector<DataPoint>> lhs = sequences.subList(0,half);
        Vector<Vector<DataPoint>> rhs = sequences.subList(half);
        /* recursively combine */
        Vector<DataPoint>lh = combine(lhs);
        Vector<DataPoint>rh = combine(rhs);
        /* merge 2 into 1 --- O(n) */
        Vector<DataPoint>final = merge(lh,rh);
        return final;
    }
}
...
Vector<DataPoint> merge(const Vector<DataPoint> &v1, const Vector<DataPoint> &v2)
{
    ...
    while(i<v1.size() || j<v2.size())
    {
        ...
        if(i==v1.size())
        {
            return final += v2.subList(j);
        }
        else if( j==v2.size())
        {
            return final += v1.subList(i);
        }
    }
    return final;
}
